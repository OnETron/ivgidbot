...
def send_main_menu(bot, message, new_message=True):
    to_sights = types.InlineKeyboardButton('О достопримечательностях',
                                           callback_data=to_callback_data({"section": 'to_location'}))
    #В связи с неработоспособностью данной функции пришлось от нее отказаться увы ;(
    '''to_add_new = types.InlineKeyboardButton(
        'Добавить новое' if message.chat.id in admins else 'Хочу предложить что-то свое',
        callback_data=to_callback_data({"section": 'to_add_new'}))'''
    to_about = types.InlineKeyboardButton('О боте', callback_data=to_callback_data({"section": 'to_about'}))

    markup = types.InlineKeyboardMarkup([[to_sights], [to_about]])

    if new_message:
        bot.send_message(message.chat.id,
                         'Привет! Я твой личный гид по достопримечательностям Ивановской области. Перейдем к делу. О чем бы ты хотел узнать?',
                         reply_markup=markup)
    else:
...
def send_location(bot, call):
    city = types.InlineKeyboardButton('Иваново', callback_data=to_callback_data(
        {"section": 'to_categories', "city": True, "page": 0}))
    region = types.InlineKeyboardButton('Ивановская область', callback_data=to_callback_data(
        {"section": 'to_categories', "city": False, "page": 0}))
    ...
    bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.id,
                          text='Давай определимся с местоположением', reply_markup=markup)


def send_categories(bot, call, city, page):
    categories = list(filter(lambda category: list(filter(lambda sight: sight.city == city, category.sights)) != [],
                             Category.select()))[page * 5:]
    ...
    for i in range(min(5, len(categories))):
        buttons.append(types.InlineKeyboardButton(categories[i].name, callback_data=to_callback_data(
            {"section": 'to_sights', "category_id": categories[i].id, "city": city, "page": 0, 'categories_page': page,
             "to_delete": None})))

    if page == 0:
        previous_button = types.InlineKeyboardButton('', callback_data='null')
    else:
        previous_button = types.InlineKeyboardButton('<<', callback_data=to_callback_data(
            {"section": 'to_categories', "city": city, "page": page - 1}))

    if len(categories) > 5:
        next_button = types.InlineKeyboardButton('>>', callback_data=to_callback_data(
            {"section": 'to_categories', "city": city, "page": page + 1}))
    else:
    ...
    bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.id, text='Выберите категорию',
                          reply_markup=markup)
...
def send_sights(bot, call, category_id, city, page, categories_page):
    ...
    for i in range(min(5, len(sights))):
        buttons.append(types.InlineKeyboardButton(sights[i].name, callback_data=to_callback_data(
            {"section": 'to_sight', "sight_id": sights[i].id, "categories_page": categories_page,
             "sights_page": page})))
    ...
    if page == 0:
        ...
        previous_button = types.InlineKeyboardButton('<<', callback_data=to_callback_data(
            {"section": 'to_sights', "categories_page": categories_page, "category_id": category_id, "city": city,
             "page": page - 1, "to_delete": None}))
    ...
    to_location = types.InlineKeyboardButton('Назад', callback_data=to_callback_data(
        {"section": 'to_categories', "page": categories_page, "city": city}))
    ...
    bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.id, text='Держи,это все достопримечательности\nв этой категории',
                          reply_markup=markup)
...
def send_sight(bot, call, sight_id, categories_page, sights_page):
    ...
    to_confirm_delete_sight = types.InlineKeyboardButton('Удалить предыдущий запрос', callback_data=to_callback_data(
        {"section": 'to_confirm_delete_sight', "sight_id": sight_id, "categories_page": categories_page,
         "sights_page": sights_page, "to_delete": messages_id}))
    to_sights = types.InlineKeyboardButton('Продолжим изучать', callback_data=to_callback_data(
        {"section": 'to_sights', "categories_page": categories_page, "page": sights_page,
         "category_id": sight.category.id, "city": sight.city, "to_delete": messages_id}))
    ...
    if call.message.chat.id in admins:
        bot.send_message(call.message.chat.id, text='Выбирай, что будем делать дальше?', reply_markup=markup_admin)
...
def send_confirm_delete_sight(bot, call, sight_id, categories_page, sights_page):
    to_sights = types.InlineKeyboardButton('Отменить', callback_data=to_callback_data(
        {'section': 'to_sight', 'sight_id': sight_id, 'categories_page': categories_page, 'sights_page': sights_page}))
    confirm_delete = types.InlineKeyboardButton('Удалить', callback_data=to_callback_data(
        {'section': 'to_delete_sight', 'sight_id': sight_id, 'categories_page': categories_page,
         'sights_page': sights_page}))
    markup = types.InlineKeyboardMarkup([[confirm_delete], [to_sights]])
    bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.id,
                          text='Вы уверены, что хотите удалить достопримечательность «' + Sight.get_by_id(
                              sight_id).name + '»?', reply_markup=markup)
...
#Эту часть кода также пришлось удалить
...
def callback_handler(bot, call):
    ...
    #И это тоже пришлось удалить в финальной версии кода из за ненадобности
...
async def error_404(bot, message):
    ...
    '''if message.reply_to_message is not None and message.reply_to_message.from_user.username == 'ivgidbot':
    ...
    to_main_menu = types.InlineKeyboardButton('В главное меню',
                                              callback_data=to_callback_data({"section": 'to_main_menu'}))
    markup = types.InlineKeyboardMarkup([[to_main_menu]])
    await bot.send_message(message.chat.id, 'Боюсь, я тебя не понимаю', reply_markup=markup)


def to_callback_data(
        uncompressed_data):  # telegram can't store big strings in callback_data (>64 bytes), so i need to compress it
    compressed_data = []
    section = uncompressed_data['section']
    sections_map = {'to_main_menu': 'mm', 'to_location': 'l', 'to_categories': 'c', 'to_sights': 'ss', 'to_sight': 's',
                    'to_add_new': 'an', "to_admin_options": 'ao', 'to_about': 'a', 'to_confirm_delete_sight': 'cds',
                    'to_delete_sight': 'ds'}
    ...
    if section == 'to_sights':
        compressed_data.extend(
            [uncompressed_data['category_id'], int(uncompressed_data['city']), uncompressed_data['page'],
             uncompressed_data['categories_page'], uncompressed_data['to_delete']])
    if section == 'to_sight':
        compressed_data.extend(
            [uncompressed_data['sight_id'], uncompressed_data['sights_page'], uncompressed_data['categories_page']])
    if section == 'to_confirm_delete_sight':
        compressed_data.extend(
            [uncompressed_data['sight_id'], uncompressed_data['sights_page'], uncompressed_data['categories_page'],
             uncompressed_data['to_delete']])
...
def from_callback_data(compressed_data):
    ...
    uncompressed_data = {}
    sections_map = {'mm': 'to_main_menu', 'l': 'to_location', 'c': 'to_categories', 'ss': 'to_sights', 's': 'to_sight',
                    'an': 'to_add_new', 'ao': 'to_admin_options', 'a': 'to_about', 'cds': 'to_confirm_delete_sight',
                    'ds': 'to_delete_sight'}
...
async def parse_message(message):
    try:
        ...
        if message.media_group_id is not None:
            images_id = [message_with_image.photo.file_id for message_with_image in
                         await app.get_media_group(message.chat.id, message.id)]
        if message.chat.id in admins:
            parsed_message = {'name': strings[0], 'address': strings[1], 'city': strings[2].lower() == 'иваново',
                              'category': Category.get_or_create(name=strings[3])[0],
                              'description': '\n'.join([*strings[4:]]), 'imgs': images_id,
                              'date_modified': date.today()}
        else:
            parsed_message = {'name': strings[0], 'address': strings[1],
...
'''async def create_new_sight(bot, message):
    ...
    if parsed_message is not None:
        if message.chat.id in admins:
            ...
            to_main_menu = types.InlineKeyboardButton('Обратно в главное меню',
                                                      callback_data=to_callback_data({"section": 'to_main_menu'}))
            markup = types.InlineKeyboardMarkup([[to_main_menu]])
            await bot.send_message(message.chat.id, 'Отлично! Я сохранил эту достопримечательность',
                                   reply_markup=markup)
        else:
            ...
            await bot.send_message(message.chat.id,
                                   'Отлично! Я переслал твое сообщение админу, рано или поздно он обязательно его посмотрит',
                                   reply_markup=markup)
    else:
        to_add_new = types.InlineKeyboardButton('Я хочу попытаться еще раз',
                                                callback_data=to_callback_data({"section": 'to_add_new'}))
        to_main_menu = types.InlineKeyboardButton('Я сдаюсь',
                                                  callback_data=to_callback_data({'section': 'to_main_menu'}))
